...
export async function fetchAppData(buildingId: string, profileId: string | null, unitId: string | null, buildings?: UserBuilding[]): Promise<AppData> {
  const [bRes, pRes, uRes, memRes, chRes, allChRes, ledRes, incRes, postRes, notifRes, docRes, agRes, allAgRes, settingsRes, insurRes, mandateRes, ruleRes, budgetRes, urgentWorksRes] = await Promise.all([
    ...
    unitId ? supabaseAdmin.from("units").select("*").eq("id", unitId).maybeSingle() : supabaseAdmin.from("units").select("*").eq("building_id", buildingId).limit(1).maybeSingle(),
    ...
    profileId ? supabaseAdmin.from("notifications").select("*").eq("profile_id", profileId).order("created_at", { ascending: false }).limit(100) : Promise.resolve({ data: [] }),
    ...
    supabaseAdmin.from("copropriete_rules").select("id, title, file_url, adopted_at, created_at, annexes").eq("building_id", buildingId).maybeSingle(),
