...
export function DocumentsView({ documents: initial, buildingId }: { documents: Doc[]; buildingId: string }) {
  ...
  const DOC_CATEGORIES = [
    ...
    { value: "facture", label: T.cats.facture, icon: "Receipt", tint: "bg-coral-100", color: "text-coral-600" },
    { value: "attestation", label: T.cats.attestation, icon: "BadgeCheck", tint: "bg-emerald-100", color: "text-emerald-600" },
    { value: "appel", label: T.cats.appel, icon: "ReceiptText", tint: "bg-orange-100", color: "text-orange-600" },
    ...
    <div>
      <PageHeader title={T.title} subtitle={T.subtitle} action={
        <button onClick={() => setShowUpload(true)} className="inline-flex items-center gap-1.5 rounded-lg bg-palier-600 px-3.5 py-2 text-[13px] font-medium text-white hover:bg-palier-700">
          <Icon name="Upload" className="h-3.5 w-3.5" /> {T.uploadBtn}
        </button>
      } />
      ...
      <div className="mb-6 grid grid-cols-2 gap-3 md:grid-cols-3">
        <KpiCard label={T.kpi.documents} value={String(totalDocs)} />
        <KpiCard label={T.kpi.categories} value={String(categories.size)} />
        <KpiCard label={T.kpi.lastAdded} value={docs.length > 0 ? shortDate(docs[0].date, lang) : "—"} />
      </div>
      ...
      <Card>
        {filtered.length > 0 ? (
            ...
            <table className="w-full table-fixed">
              <thead>
                <tr className="border-b border-black/[0.06] text-[11px] font-semibold uppercase tracking-wide text-ink-faint">
                  ...
                  <th className="w-[35%] pb-2 text-left">{T.table.document}</th>
                  <th className="w-[18%] pb-2 text-left">{T.table.category}</th>
                  <th className="w-[14%] pb-2 text-left">{T.table.date}</th>
                  <th className="w-[10%] pb-2 text-left">{T.table.size}</th>
                  <th className="w-[18%] pb-2 text-right">{T.table.actions}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((doc) => {
                  ...
                  return (
                    <tr key={doc.id} className="border-b border-black/[0.04] last:border-0">
                      ...
                      <td className="py-3 text-[13px] text-ink-soft" dir="ltr">{shortDate(doc.date, lang)}</td>
                      <td className="py-3 text-[13px] text-ink-soft" dir="ltr">{doc.size || "—"}</td>
                ...
                <div key={doc.id} className="flex items-center gap-3 p-4">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-0.5 flex items-center gap-2 text-[12px] text-ink-soft">
                      ...
                      <span dir="ltr">{shortDate(doc.date, lang)}</span>
                      {doc.size && <span dir="ltr">{doc.size}</span>}
                    </div>
                  </div>
          ...
          <div className="py-10 text-center">
            ...
            <p className="text-[13px] text-ink-soft">{search || catFilter !== "all" ? T.noDocuments : T.noDocumentsAdded}</p>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center px-4 bg-black/30" onClick={() => setShowUpload(false)}>
          <div className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-2xl border border-black/[0.06] bg-cream-card p-5 shadow-card" onClick={(e) => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              <h2 className="text-[16px] font-semibold text-ink">{T.upload.title}</h2>
            ...
            {/* Title */}
            <label className="mb-1 block text-[12px] font-medium text-ink-soft">{T.upload.titleLabel}</label>
            <input
              value={upTitle}
            ...
            {/* Category */}
            <label className="mb-1 block text-[12px] font-medium text-ink-soft">{T.upload.category}</label>
            <select
              value={upCategory}
            ...
            {/* File drop zone */}
            <label className="mb-1 block text-[12px] font-medium text-ink-soft">{T.upload.fileLabel}</label>
            ...
            >
              {upFile ? (
                <>
                  ...
                  <p className="text-[11px] text-ink-soft" dir="ltr">{(upFile.size / 1024).toFixed(0)} KB</p>
                </>
              ) : (
                <>
                  <Icon name="Upload" className="mb-1 h-6 w-6 text-ink-soft" />
                  <p className="text-[13px] font-medium text-ink">{T.upload.selectFile}</p>
                  <p className="text-[11px] text-ink-soft">{T.upload.fileTypes}</p>
                </>
              )}
            </button>
